    ...
    on: ((event: string, fn: (...args: unknown[]) => unknown): Unsubscribe => {
      if (event === 'valueChange') {
        ...
        if (import.meta.env.DEV) {
          ...
          console.warn(
            '[extension-api] widget.on("beforeQueue") is not yet wired — Phase B'
          )
        }
        dispatch({
      ...
      if (import.meta.env.DEV) {
        ...
        console.warn(
          `[extension-api] widget.on("${event}") is not a valid event name`
        )
      }
      return () => {}
    }) as WidgetHandle['on']
  ...
  return {
    ...
    on: ((event: string, fn: (...args: unknown[]) => unknown): Unsubscribe => {
      ...
      if (import.meta.env.DEV) {
        ...
        console.warn(
          `[extension-api] node.on("${event}") is not a valid event name`
        )
      }
      return () => {}
    }) as NodeHandle['on']
